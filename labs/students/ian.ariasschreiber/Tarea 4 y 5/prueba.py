from flask import Flask,render_template,session,request, jsonify, Response
from model import entities
from database import connector
import json




app = Flask(__name__)
db = connector.Manager()

cache = {}
engine = db.createEngine()

@app.route('/')
def hello_world():
    return render_template('login.html')

@app.route('/dologin',  methods = ['POST'])
def do_login():

    data = request.form
    session = db.getSession(engine)
    users = session.query(entities.User)
    for user in users:
        if user.name == data['username'] and user.password == data['password']:
            return render_template('chat.html')
    return render_template('login.html')

@app.route('/setUsers')
def set_user():

    user1 = entities.User(id=1, name='ed', fullname='Ed Jones', password='hola123')
    user2 = entities.User(id=2, name='jb', fullname='Je Belli', password='bye123')
    user3 = entities.User(id=3, name='Freddy', fullname='Fredward Benson', password='ICarly')
    session = db.getSession(engine)
    session.add(user1)
    session.add(user2)
    session.add(user3)
    session.commit()
    return 'Created users'

@app.route('/users', methods = ['GET'])
def get_users():
    key = 'getUsers'
    if key not in cache.keys():
        session = db.getSession(engine)
        dbResponse = session.query(entities.User)
        cache[key] = dbResponse;
        print("From DB")
    else:
        print("From Cache")

    users = cache[key];
    response = []
    for user in users:
        response.append(user)
    return json.dumps(response, cls=connector.AlchemyEncoder)

@app.route('/users/<id>', methods = ['GET'])
def get_user(id):
    session = db.getSession(engine)
    users = session.query(entities.User).filter(entities.User.id == id)
    for user in users:
        js = json.dumps(user, cls=connector.AlchemyEncoder)
        return  Response(js, status=200, mimetype='application/json')

    message = { "status": 404, "message": "Not Found"}
    return Response(message, status=404, mimetype='application/json')


@app.route('/users/<id>', methods = ['DELETE'])
def remove_user(id):
    session = db.getSession(engine)
    users = session.query(entities.User).filter(entities.User.id == id)
    for user in users:
        session.delete(user)
    session.commit()
    return "DELETED"


@app.route('/users', methods = ['POST'])
def create_user():
    c = request.get_json(silent=True)
    print(c)
    user = entities.User(
        id=c['id'],
        name=c['name'],
        fullname=c['fullname'],
        password=c['password']
    )
    session = db.getSession(engine)
    session.add(user)
    session.commit()
    return 'Created users'


@app.route('/message', methods=['GET'])
def get_msgs():
    key = 'getMsgs'
    if key not in cache.keys():
        session = db.getSession(engine)
        dbResponse = session.query(entities.msg)
        cache[key] = dbResponse;
        print("De la base de datos")
    else:
        print("Del cache")

    messages = cache[key]
    list_messages = []
    for msg in messages:
        list_messages.append(msg)
    return json.dumps(list_messages, cls=connector.AlchemyEncoder)


@app.route('/message/<id>', methods=['GET'])
def get_msg(id):
    session = db.getSession(engine)
    messages = session.query(entities.msg).filter(entities.msg.id == id)
    for msg in messages:
        js = json.dumps(msg, cls=connector.AlchemyEncoder)
        return Response(js, status=200, mimetype='application/json')

    output = {"status": 404, "message": "Not Found"}
    return Response(output, status=404, mimetype='application/json')


@app.route('/message/<id>', methods=['DELETE'])
def delete_msg(id):
    session = db.getSession(engine)
    messages = session.query(entities.msg).filter(entities.msg.id == id)
    for msg in messages:
        session.delete(msg)
    session.commit()
    return "El mensaje ha sido eliminado"


@app.route('/message', methods=['POST'])
def new_msg():
    c = request.get_json(silent=True)
    print(c)
    message = entities.msg(
        id=c['id'],
    )
    session = db.getSession(engine)
    session.add(message)
    session.commit()
    return 'El mensaje ha sido creado'


@app.route('/message/<id>', methods=['PUT'])
def act_msg(id):
    delete_msg(id)
    new_msg()
    return 'El mensaje ha sido actualizado'


if __name__ == '__main__':
    app.run()

