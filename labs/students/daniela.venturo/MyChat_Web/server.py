...
def getAllUsers():
    if request.method == "GET":
        ...
        if accept == "application/xml":
            users = "<users>"
            for x in userList:
                users += '<User><id>' + str(x.id) + '</id> <name>' + str(x.name) + '</name> <fullname>' + str(
                    x.fullname) + '</fullname> <password>' + str(x.password) + '</password></User>'
            users += '</users>'

            return users

        elif "application/json" in accept: #asi funciona con el devextreme que tiene json y mas cosas, y funciona con postman que es solo application json
            users = "["
            for x in userList:
                users += '{"id":' + str(x.id) + ',"name":"' + str(x.name) + '","fullname":"' + str(
                    x.fullname) + '","password":"' + str(x.password) + '"},'
